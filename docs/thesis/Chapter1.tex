...
\section{Acronimi}
E' possibile usare il package "glossaries" per riportare automaticamente gli acronimi nell'apposito indice.
Per aggiungere nuovi acronimi è sufficiente aprire nel file acronyms.tex e usare il seguente comando:
\begin{center}
\textbackslash newacronym\{acronym\}\{ACRONYM\}\{Definition\}
\end{center}
Esempio di utilizzo:
\begin{center}
    \textbackslash newacronym\{vr\}\{VR\}\{Virtual Reality\}
\end{center}
Essi possono essere richiamati nel testo nel seguente modo:
\begin{itemize}
    \item Comando \textbackslash glsxtrfull\{vr\}: richiama sia la definizione dell'acronimo che l'acronimo stesso (es. \glsxtrfull{vr})
    \item Comando \textbackslash glsxtrlong\{vr\}: richiama la sola definizione dell'acronimo (es. \glsxtrlong{vr})
    \item Comando \textbackslash glsxtrshort\{vr\}: richiama il solo acronimo (es. \glsxtrshort{vr})
\end{itemize}
Aggiungendo il suffisso "pl" ai suddetti comandi è possibile ottenere la versione plurale della definizione e dell'acronimo:
\begin{itemize}
    \item Esempio di output del comando \textbackslash glsxtrfullpl\{vr\}: \glsxtrfullpl{vr}
    \item Esempio di output del comando \textbackslash glsxtrlongpl\{vr\}: \glsxtrlongpl{vr}
    \item Esempio di output del comando \textbackslash glsxtrshortpl\{vr\}: \glsxtrshortpl{vr}
\end{itemize}
Com'è possibile notare, per alcuni casi il plurale di una parola, ottenuto aggiungendo la sola s al termine, potrebbe non essere corretto. In tal caso, occorre dichiarare il plurale corretto nel seguente modo:
\begin{center}
\textbackslash newacronym[shortplural=\{VRs\},longplural=\{Virtual Realities\}]\{vr\}\{VR\}\{Virtual Reality\}
\end{center}
Seguono esempi di plurali di definizioni di acronimi e acronimi corretti:
\begin{itemize}
    \item Esempio di output del comando \textbackslash glsxtrfullpl\{ghg\}: \glsxtrfullpl{ghg}
    \item Esempio di output del comando \textbackslash glsxtrlongpl\{ghg\}: \glsxtrlongpl{ghg}
    \item Esempio di output del comando \textbackslash glsxtrshortpl\{ghg\}: \glsxtrshortpl{ghg}
\end{itemize}
