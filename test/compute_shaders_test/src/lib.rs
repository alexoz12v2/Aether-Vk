...
impl VulkanContext {
    pub fn new() -> Self {
        ...
        Self { frontend, device_handle, _pool: pool_arc }
    }

    pub fn create_buffer<T>(&self, data: &[T], usage: vk::BufferUsageFlags) -> (vk::Buffer, vk_mem::Allocation, u64) {
        self.frontend.with_device(self.device_handle, |dev| {
            ...
            let (buffer, alloc, info) = unsafe { allocator.create_buffer_get_info(&buffer_info, &alloc_info) }.unwrap();

            if size > 0 {
                unsafe {
                    std::ptr::copy_nonoverlapping(data.as_ptr() as *const u8, info.mapped_data as *mut u8, size as usize);
                }
            }

            let device_address_info = vk::BufferDeviceAddressInfo::default().buffer(buffer);
            let address = unsafe { actual_device.device.buffer_device_address.get_buffer_device_address(&device_address_info) };
    ...
    pub fn read_buffer<T: Copy>(&self, _buffer: vk::Buffer, alloc: &mut vk_mem::Allocation, count: usize) -> Vec<T> {
        self.frontend.with_device(self.device_handle, |dev| {
            let actual_device = dev.as_any().downcast_ref::<Device>().unwrap();
            let allocator = &*actual_device.res.read().allocator.allocator;
...
pub fn run_compute_shader(
    ...
    ctx.frontend.with_device(ctx.device_handle, |dev| {
        ...
        let pipelines = unsafe {
            ash_dev.create_compute_pipelines(vk::PipelineCache::null(), std::slice::from_ref(&compute_info), None)
        }.unwrap();
