...
fn test_p3_4_imex_rigidbody_imr() {
  ...
  let output_bodies: Vec<f32> =
    ctx.read_buffer(rb_buffer, &mut rb_alloc, test_data.rigid_bodies.len());

  ctx.destroy_buffer(rb_buffer, rb_alloc);
  ctx.destroy_buffer(emitters_buffer, emitters_alloc);

  for (i, (actual, expected)) in
    output_bodies.iter().zip(test_data.expected_rigid_bodies.iter()).enumerate()
  {
