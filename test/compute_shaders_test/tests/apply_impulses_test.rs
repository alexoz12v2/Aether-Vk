...
fn test_apply_impulses() {
  compute_shaders_test::ensure_test_data(
    "test_data/apply_impulses.json",
    "gen_apply_impulses_data.py",
  );
  ...
  let collision_count = test_data.collisions[3];
  let dispatch_x = (collision_count + 127) / 128;

  // Run the compute shader
  run_compute_shader(&ctx, spv_path, push_constants_bytes, dispatch_x, 1, 1);

  let output_particles: Vec<f32> = ctx.read_buffer(
    particles_buffer,
    &mut particles_alloc,
    test_data.particles.len(),
  );
  ...
  for (i, (actual, expected)) in
    output_particles.iter().zip(test_data.expected_particles.iter()).enumerate()
  {
