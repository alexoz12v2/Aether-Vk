...
fn test_lbvh_build() {
    ...
    let mut particles = vec![0.0f32; (10 * subgroup_size * ((num_primitives + subgroup_size - 1) / subgroup_size)) as usize];
    for i in 0..num_primitives {
    ...
    let bvh_nodes = vec![BVHNodeAABB {
        bound: AABB { min_bounds: [0.0; 3], max_bounds: [0.0; 3] },
    ...
    let output_bvh: Vec<BVHNodeAABB> = ctx.read_buffer(bvh_buffer, &mut bvh_alloc, total_nodes as usize);
    ...
    for i in 0..(num_primitives - 1) as usize {
        assert_eq!(output_bvh[i].primitive_count, 0, "Internal node {} should have primitive_count = 0", i);
        assert!(output_bvh[i].mass > 0.0, "Internal node {} mass should be > 0.0", i);
    }

    // Leaf nodes should have primitive_count = 1
    for i in (num_primitives - 1) as usize..total_nodes as usize {
        assert_eq!(output_bvh[i].primitive_count, 1, "Leaf node {} should have primitive_count = 1", i);
        assert_eq!(output_bvh[i].mass, 10.0, "Leaf node {} mass should be 10.0", i);
    }
}
