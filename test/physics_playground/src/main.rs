...
{
  let mut scene = Scene::new(std::sync::Arc::new(aethervk_core_rlib::gpu::RwLock::new(
...
fn test_kernels_5s() {
  ...
  let simd_kernels = CpuSimdKernels {
...
fn test_async_time_manipulation() {
  ...
  let scene_ctx =
    aethervk_core_rlib::simulation_api::structs::SceneContext::new_empty(scene_arc.clone(), root)
      .with_physics_scene();
  ...
  let _ = logic_tx.try_send(LogicCommand::SetSceneTimeScale {
    scene_id: 1,
    scale: TimeScale::OneDay,
  });
  ...
  std::thread::sleep(std::time::Duration::from_millis(50));
  {
    ...
    let scene_ctx = scenes_read.get(&1).unwrap().read();
    assert_eq!(
      scene_ctx.time_state.read().current_scale,
      TimeScale::OneWeek
    );
  }
