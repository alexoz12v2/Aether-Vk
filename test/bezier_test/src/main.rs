...
use aethervk_core_rlib::scene::{
  trajectory::TrajectoryComponent, CameraComponent, Scene, TransformComponent,
};
...
fn main() {
  *aethervk_core_rlib::gpu::ASSET_DIR.write() = Some(cycle_get_asset_path_from_exe(true).to_string_lossy().to_string());
  ...
  {
    let mut event_loop_builder = winit::event_loop::EventLoopBuilder::<test_utils::AppEvent>::with_user_event();
    #[cfg(target_os = "macos")]
    {
  ...
  let presentation_engine = {
    #[cfg(feature = "windowed")]
    {
      ...
      let (native_handles, window_info) =
        test_utils::get_handle_and_window_info(&render_frontend, render_device_handle, window);
  ...
  for i in 0..10_000 {
    ...
    let traj_e = scene.spawn_entity(&format!("orbit_{}", i));

    let mut cps = std::vec::Vec::new();
    // Just make a dummy square-ish orbit for now using cubic segments
    cps.push([-1.0, 0.0, -1.0, 1.0]);
    cps.push([0.0, 0.0, -2.0, 1.0]);
    cps.push([2.0, 0.0, 0.0, 1.0]);
    cps.push([1.0, 0.0, 1.0, 1.0]);
    ...
    scene
      .add_component(
        traj_e,
        TransformComponent {
          position: Vec3f32::from_array([
            (rng % 100) as f32 * 0.1 - 5.0,
            0.0,
            (rng % 50) as f32 * 0.1 - 2.5,
          ]),
          rotation: Quat::identity(),
          scale: Vec3f32::from_array([1.0, 1.0, 1.0]),
        },
    ...
    scene
      .add_component(
        ...
        TrajectoryComponent {
          control_points: cps,
          color: [((rng % 255) as f32) / 255.0, 1.0, 0.0, 1.0],
          line_width: 2.0,
          texture_id: 0,
  ...
  let mut frames = 0;
  let mut time_readings = aethervk_oshal_rlib::os::time::TimeReadings::default();

  let render_frame_fn = |device: &dyn aethervk_core_rlib::gpu::RenderDevice, width: u32, height: u32, is_windowed: bool, frame_count: u32| -> aethervk_core_rlib::types::GpuResult<()> {
    ...
    let extracted = scene.convert_scene(cam_e, false, Some(&*pool_arc), [width, height]).unwrap();
    ...
    device.set_viewport(cmd_buffer_handle, &aethervk_core_rlib::gpu::Viewport::from_extent([width, height]))?;
    device.set_scissor(cmd_buffer_handle, &aethervk_core_rlib::gpu::Rect2D::from_extent([width, height]))?;
        ...
        Event::WindowEvent {
          ...
          ..
            ...
            if size.width > 0 && size.height > 0 {
                render_frontend.with_device(render_device_handle, |device| {
                    device.resize_presentation_engine(presentation_engine, size.width, size.height)
                }).unwrap();
            }
        },
        _ => (),
