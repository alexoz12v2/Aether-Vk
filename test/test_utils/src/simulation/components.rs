...
pub trait ParticleSystemExt {
  fn update_particles(
    &self,
    config: &aethervk_core_rlib::scene::particles::ParticleEmitterComponent,
    dt: f32,
    sun_pos: Vec3f32,
    comet_pos: Vec3f32,
    comet_mass: f32,
...
impl ParticleSystemExt for ParticleSystemComponent {
  fn update_particles(
    ...
    for p in self.particles.write().iter_mut().filter(|p| p.active != 0) {
      ...
      // Russian roulette
      if age > config.lifetime as i64 {
        let age_excess = (age - config.lifetime as i64) as f32 / 1_000_000.0;
    ...
    if self.particles.read().len() > config.max_particles {
