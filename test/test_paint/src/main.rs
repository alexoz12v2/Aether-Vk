...
struct PaintDelegate {
  ...
  pending_raycast: Option<core::num::NonZeroU64>,
}
...
impl PaintDelegate {
  ...
  fn update_ui_text(&self, ctx: &mut SimulationContext, scene_id: u64) {
    if let Some(ui_e) = self.ui_text_entity {
      ...
      if let Some(mesh_id) = self.mesh_entity {
        active_scene
          .scene
          .with_component_mut::<aethervk_core_rlib::scene::PhysicalMeshComponent, _, _>(
            mesh_id,
            |c| {
  ...
  fn apply_paint(&mut self, ctx: &mut SimulationContext, scene_id: u64) {
    ...
    if let (Some(cam), Some(mesh_id)) = (self.camera_entity, self.mesh_id) {
      if let Some(task_id) = self.pending_raycast {
        if ctx.task_manager.read().get_status(task_id.get()) == TaskStatusCode::Pending {
          return;
        }
        self.pending_raycast = None;
        let task_result = ctx.task_manager.write().take_result(task_id.get());
        let hit_res = match task_result {
          Some(SimulationTaskResult::Raycast(hit)) => hit,
          _ => None,
        };
        ...
        if let Some(hit) = hit_res {
          if let Some(cursor_e) = self.cursor_entity {
            ...
            let _ = active_scene
              .scene
              .remove_component::<aethervk_core_rlib::scene::HiddenComponent>(cursor_e);
          }
                ...
                if dist <= r_px {
                  ...
                  unsafe {
                    if self.submode == Submode::Color {
                      ...
                      let target_r = if self.is_shift_down {
                        128.0 / 255.0
                      } else {
                        self.color_val[0]
                      };
                      let target_g = if self.is_shift_down {
                        128.0 / 255.0
                      } else {
                        self.color_val[1]
                      };
                      let target_b = if self.is_shift_down {
                        128.0 / 255.0
                      } else {
                        self.color_val[2]
                      };
          ...
          if let Some(cursor_e) = self.cursor_entity {
            let _ = ctx
              ...
              .write()
              .scene
              .add_component(cursor_e, aethervk_core_rlib::scene::HiddenComponent {});
          }
        }
      }

      if self.is_left_mouse_down && self.pending_raycast.is_none() {
        let scene_arc = ctx.get_scene(scene_id).unwrap();
        ...
        if let Ok(task_id) = ctx.raycast_ndc(scene_id, camera_entity, ndc_x, ndc_y) {
          self.pending_raycast = Some(task_id);
        }
      }
    }
  }
...
impl SimulationDelegate for PaintDelegate {
  ...
  fn on_setup(
    ...
    let mesh_instance_id =
      aethervk_core_rlib::gpu::RenderableInstanceId::from_physical_mesh(comet_arc.id);
  ...
  fn on_keyboard_input(
    ...
    if event.state == ElementState::Pressed {
      ...
      println!(
        "DEBUG: on_keyboard_input key pressed: {:?}",
        event.physical_key
      );
      match event.physical_key {
        ...
        PhysicalKey::Code(KeyCode::Digit0) => {
          if let (Some(cam_e), Some(center_e)) = (self.camera_entity, self.view_center) {
            ...
            let cur_pos = active_scene
              .scene
              .with_component(
                center_e,
                |t: &aethervk_core_rlib::scene::TransformComponent| t.position,
              )
              .unwrap_or(Vec3f32::from_array([0.0, 0.0, 0.0]));
