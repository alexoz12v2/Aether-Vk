  ...
  Self::Vector: Vector4,
{
  ...
  fn to_quat_custom_frame<Q>(&self) -> Q
  where
    Q: Quaternion<Scalar = Self::Scalar>,
    Q::Vector: Vector3,
    Self::Scalar: FloatLike,
  {
    let m00 = unsafe { self.column_unchecked(0).x() };
    let m01 = unsafe { self.column_unchecked(1).x() };
    let m02 = unsafe { self.column_unchecked(2).x() };
    let m10 = unsafe { self.column_unchecked(0).y() };
    let m11 = unsafe { self.column_unchecked(1).y() };
    let m12 = unsafe { self.column_unchecked(2).y() };
    let m20 = unsafe { self.column_unchecked(0).z() };
    let m21 = unsafe { self.column_unchecked(1).z() };
    let m22 = unsafe { self.column_unchecked(2).z() };
    ...
    if trace > _0 {
      ...
      Q::from_vector_and_scalar(
        Q::Vector::from_components(
          (m21 - m12) * inv_s,
          (m02 - m20) * inv_s,
          (m10 - m01) * inv_s,
        ),
      ...
      let inv_s: Self::Scalar = _1 / s;
      Q::from_vector_and_scalar(
        Q::Vector::from_components(_0_25 * s, (m01 + m10) * inv_s, (m02 + m20) * inv_s),
        (m21 - m12) * inv_s,
      )
    } else if m11 > m22 {
      ...
      let inv_s: Self::Scalar = _1 / s;
      Q::from_vector_and_scalar(
        Q::Vector::from_components((m01 + m10) * inv_s, _0_25 * s, (m12 + m21) * inv_s),
        (m02 - m20) * inv_s,
      )
    } else {
      ...
      let inv_s: Self::Scalar = _1 / s;
      Q::from_vector_and_scalar(
        Q::Vector::from_components((m02 + m20) * inv_s, (m12 + m21) * inv_s, _0_25 * s),
        (m10 - m01) * inv_s,
      )
    }
  }
  ...
  /// Creates a Vulkan-ready orthographic projection matrix with reverse-Z mapping [1, 0]
  /// +x = right, -y = forward, +z = up
  fn orthographic_vk_reverse_z(
    left: Self::Scalar,
    right: Self::Scalar,
    bottom: Self::Scalar,
    top: Self::Scalar,
  ...
  /// Column major perspective projection matrix for coordinate system:
  /// +x = right, -y = forward, +z = up
  /// Reverse-Z mapping: Depth is mapped to [1, 0] instead of [0, 1]
  fn perspective_vk_reverse_z(
    fov: Self::Scalar,
    aspect: Self::Scalar,
    near: Self::Scalar,
    far: Self::Scalar,
    ...
    Self::Scalar: FloatLike,
  {
    ...
    let c0 = Self::Vector::from_components(f / aspect, _0, _0, _0);
    let c1 = Self::Vector::from_components(_0, _0, near / (far - near), -_1);
    let c2 = Self::Vector::from_components(_0, -f, _0, _0);
    let c3 = Self::Vector::from_components(_0, _0, far * near / (far - near), _0);
