                                            ...
                                            if let Ok(num_frames_padding) = client.GetCurrentPadding() {
                                                ...
                                                if num_frames_available > 0 {
                                                    if let Ok(buffer_ptr) = render_client.GetBuffer(num_frames_available) {
                                                        let num_samples = (num_frames_available as usize) * (mix_format.nChannels as usize);
                                                        let slice = core::slice::from_raw_parts_mut(buffer_ptr as *mut f32, num_samples);
...
pub mod macos_coreaudio {
    ...
    unsafe extern "C" {
        fn AudioComponentFindNext(inComponent: *mut c_void, inDesc: *const AudioComponentDescription) -> *mut c_void;
        fn AudioComponentInstanceNew(inComponent: *mut c_void, outInstance: *mut *mut c_void) -> i32;
        fn AudioUnitSetProperty(inUnit: *mut c_void, inID: u32, inScope: u32, inElement: u32, inData: *const c_void, inDataSize: u32) -> i32;
    ...
    impl AudioDevice for CoreAudioDevice {
        fn start(&mut self, render_callback: fn(&mut [f32])) {
            ...
            unsafe {
                ...
                if AudioComponentInstanceNew(comp, &mut self.unit) != 0 { return; }
