...
[Collection("Sequential")] // Prevents concurrent Vulkan init with SpawnCometFlowE2ETests
public class ViewportTrajectoryIntegrationTests
  : IRecipient<AetherVk.Logic.Messages.RenderFrameReadyMessage>
{
  private ulong _latestRenderGeneration = 0;
  private ulong _peId = 0;

  void IRecipient<AetherVk.Logic.Messages.RenderFrameReadyMessage>.Receive(
    AetherVk.Logic.Messages.RenderFrameReadyMessage message
  )
  {
    if (_peId != 0 && message.PresentationEngineId != _peId)
      return;
  ...
  public async Task OrthographicTrajectoryPixelTest()
  {
    ...
    // Registration moved after peId is known (see below)
    ...
    try
    {
      bool downloaded = await runtimeService.DownloadImageAsync(
        _latestRenderGeneration,
        buffer,
        bufferSize
      );
