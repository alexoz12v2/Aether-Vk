...
void MacosApplication::RTdoOnSurfaceLost() {
  showErrorScreenAndExit("Invalid for desktop");
}

void MacosApplication::RTdoEarlySurfaceRegained() {
  showErrorScreenAndExit("Invalid for desktop");
}

void MacosApplication::RTdoLateSurfaceRegained() {
  showErrorScreenAndExit("Invalid for desktop");
}

void MacosApplication::RTdoOnDeviceLost() {
  showErrorScreenAndExit("Invalid for desktop");
}

void MacosApplication::RTdoOnDeviceRegained() {
  showErrorScreenAndExit("Invalid for desktop");
}

// ---------------- TODO Later ---------------------------

void MacosApplication::doOnSaveState() {}

void MacosApplication::doOnRestoreState() {}

// ---------------- Other ------------------------

MacosApplication::MacosApplication() = default;

void MacosApplication::RTdoOnWindowInit() AVK_NO_CFI {
#define PREFIX "[MacosApplication::onWindowInit] "
...
void MacosApplication::UTdoOnInit() {
...
void MacosApplication::signalDisplayLinkReady() {
  m_displayLinkReady.store(true, std::memory_order_release);
}

vk::SurfaceSpec MacosApplication::doSurfaceSpec() {
  vk::SurfaceSpec spec{};
  spec.layer = MetalLayer;
  assert(MetalLayer && "Where is my metal layer");
  return spec;
}

// ------------ Cleanup Functions -----------------------

MacosApplication::~MacosApplication() noexcept AVK_NO_CFI {
...
void MacosApplication::destroyConstantVulkanResources() AVK_NO_CFI {
  // template and layout
...
void MacosApplication::cleanupVulkanResources() AVK_NO_CFI {
  using namespace avk::literals;
...
void MacosApplication::createConstantVulkanResources() AVK_NO_CFI {
  using namespace avk::literals;
  ...
  auto const path = getResourcePath() / "assets" / "starry-night-uastc.ktx2";
  ...
  // --------------------------- Main Graphics Pipeline --------------------
  {
    ...
    auto const vertCode = openSpirV(shadersPath / "cube-buffers.vert.spv");
    auto const fragCode = openSpirV(shadersPath / "cube-buffers.frag.spv");
  ...
  // ------------------------- Skybox Graphics Pipeline --------------------
  {
    ...
    {
      auto const shadersPath = getResourcePath() / "shaders";
      auto const skyboxVertCode = openSpirV(shadersPath / "skybox.vert.spv");
      auto const skyboxFragCode = openSpirV(shadersPath / "skybox.frag.spv");
...
void MacosApplication::createVulkanResources() AVK_NO_CFI {
  using namespace avk::literals;
...
void MacosApplication::UTdoOnFixedUpdate() {}

void MacosApplication::UTdoOnUpdate() {
  ...
  // wait for display link before kicking render thread
  {
    ...
    signalStateUpdated();
    m_displayLinkReady.store(false, std::memory_order_release);
  }
}

VkResult MacosApplication::RTdoOnRender(
    const vk::utils::SwapchainData& swapchainData) AVK_NO_CFI {
  using namespace avk::literals;
  auto const* const vkDevApi = vkDevTable();
...
void MacosApplication::RTdoOnResize() {
  LOGI << "[MacosApplication] Resize callback on derived class" << std::endl;
  cleanupVulkanResources();
  createVulkanResources();
}
