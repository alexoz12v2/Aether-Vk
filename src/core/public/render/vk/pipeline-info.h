...
enum class EPipelineFlags : uint32_t {
  eDepthBias = 1u << 0,
  eCull = 1u << 1,
  eCullFront = (1u << 2) | static_cast<uint32_t>(eCull),
  eCullBack = (0u << 2) | static_cast<uint32_t>(eCull),
...
inline EPipelineFlags operator|(EPipelineFlags a, EPipelineFlags b) {
  using U = std::underlying_type_t<EPipelineFlags>;
  return static_cast<EPipelineFlags>(static_cast<U>(a) | static_cast<U>(b));
}
inline EPipelineFlags operator&(EPipelineFlags a, EPipelineFlags b) {
  using U = std::underlying_type_t<EPipelineFlags>;
  return static_cast<EPipelineFlags>(static_cast<U>(a) & static_cast<U>(b));
...
inline bool hasFlag(EPipelineFlags value, EPipelineFlags flag) {
  using U = std::underlying_type_t<EPipelineFlags>;
...
// Drives stencil compare operation during pipelime creation
...
struct VertexIn {
  ...
  inline bool operator==(VertexIn const& other) const {
    ...
    auto constexpr bindingEqual = [](VkVertexInputBindingDescription const& a,
  ...
  uint64_t hash() const {
    ...
    hash =
        hash * 33 ^
        vectorHash(attributes, [](VkVertexInputAttributeDescription const& x) {
...
namespace avk::vk {
// ------------------- New Pipeline Stable Identification ---------------------
/// Identifies what a material/shader is, and it's not affected by renderpass,
/// pipeline, swapchain changes.
/// It doesn't own any of the handles it contains
struct MaterialSignature {
  VkShaderModule vertexShaderModule;
  VkShaderModule fragmentShaderModule;
  // for now we support 1 push constant (camera on 3D)
