...
class Scheduler {
 ...
 public:
  ...
  void shutdown() {
    ...
    for (size_t s = 0; s < m_totalFibers; ++s) {
      pushTask(nullptr, JobPriority::High);
    }
  ...
  void submitTask(Job* task, uint32_t depCount = 0,
                  Job** dependencies = nullptr) {
    task->m_remainingDependencies = depCount;
    for (uint32_t i = 0; i < depCount; ++i) {
      dependencies[i]->m_continuations.push_back(task);
    }
    if (depCount == 0) {  // push only if no deps
 ...
 private:
  void pushTask(Job* task, JobPriority prio) {
    if (task != reinterpret_cast<Job*>(Sentinel))
      m_inflightTasks.fetch_add(1, std::memory_order_acq_rel);
    m_queues[static_cast<size_t>(prio)]->push(task);
  }
  ...
  void fiberLoop() {
    while (true) {
      ...
      for (auto* cont : task->m_continuations) {
        if (--cont->m_remainingDependencies == 0) {
...
#ifdef AVK_DEBUG
#define JOB_NAME(sched, task) (sched).getTaskName(task)
#else
#define JOB_NAME(sched, task) (std::to_string(boost::this_fiber::get_id()))
#endif
...
void mathStep1(void* userData) {
  auto* p = static_cast<Payload*>(userData);
  p->value *= 2;
  std::cout << "[Fiber " << boost::this_fiber::get_id()
            << "] Step1: doubled to " << p->value << std::endl;
}

void mathStep2(void* userData) {
  auto* p = static_cast<Payload*>(userData);
  p->value += 1;
  std::cout << "[Fiber " << boost::this_fiber::get_id()
            << "] Step2: added one \xE2\x86\x92 " << p->value << std::endl;
}

// Build continuation chain: Step1 → Step2
void mathJobWithContinuations(Scheduler& sched, Payload* payload) {
  auto* job1 = new Job();
  job1->fn = mathStep1;
  job1->data = payload;
