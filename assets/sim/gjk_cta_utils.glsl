...
#ifdef DEBUG_SHADERS
        debugPrintfEXT("TRAP! EPA entered with invalid simplex count: %d\n", polytope_count);
#endif
    ...
    for (int iter = 0; iter < 16; ++iter) {
        ...
        if (d - min_dist < 1e-4) {
            ...
            if (abs(denom) >= 1e-6) {
                ...
                debugPrintfEXT("TRAP! EPA Degenerate face denom=%f in exit condition.\n", denom);
            }
#endif
            float u = 1.0 - v - w;
        ...
        if (polytope_count >= 12) {
#ifdef DEBUG_SHADERS
            debugPrintfEXT("TRAP! EPA polytope_points overflow! count=%d\n", polytope_count);
#endif
            break;
        }
                ...
                for (int j = 0; j < 3; ++j) {
                    ...
                    if (!found) {
                        if (num_edges < 24) {
                            ...
                            debugPrintfEXT("TRAP! EPA edges array overflow!\n");
                        }
#endif
                    }
                }
        ...
        for (int k = 0; k < num_edges; ++k) {
            if (num_faces < 16) {
                ...
                debugPrintfEXT("TRAP! EPA faces array overflow building new face!\n");
            }
#endif
        }
    }
...
// Do-Simplex implementation (Unmodified structurally, strictly math)
bool do_simplex(inout Simplex simplex, inout vec3 dir) {
    // ... [Content identical to user prompt to save tokens, strictly math] ...
    // Note: Do-simplex inherently bounds array indices correctly if fed valid simplex.count
    if (simplex.count == 2) {
...
#ifdef DEBUG_SHADERS
        // Critical Trap: If a malformed geometry state manages to bypass do_simplex's length checks
        if (simplex.count >= 4) {
            debugPrintfEXT("TRAP! GJK Generic simplex array overflow imminent! count=%d\n", simplex.count);
            break;
        }
#endif
...
#ifdef DEBUG_SHADERS
        if (simplex.count >= 4) {
            debugPrintfEXT("TRAP! GJK Spheres simplex array overflow imminent! count=%d\n", simplex.count);
            break;
        }
#endif
