...
pub trait RenderCompute: Send + Sync {
  ...
  fn dispatch_sun_volume_generation(&self, cmd_buffer: CommandBufferHandle, resolution: (u32, u32, u32)) -> GpuResult<()>;

  /// Generates the procedural skybox (e.g. using octahedral mapping) and transitions the output image.
  fn dispatch_sky_generation(&self, cmd_buffer: CommandBufferHandle, resolution: (u32, u32)) -> GpuResult<()>;
