...
/// Thread-local storing the watchdog buffer from the most recent `build_motion_bvh` call.
/// After `vkQueueWaitIdle`, call [`check_bvh_watchdog`] to assert no shader safety-counter tripped.
///
/// The stored tuple is `(VkBuffer, VmaAllocation, device_address)`.
/// This is only present in `#[cfg(test)]` builds.
#[cfg(test)]
thread_local! {
...
///
/// Call this **after** `vkQueueWaitIdle` / timeline semaphore wait, when the GPU has
/// finished executing the command buffer that contained `build_motion_bvh`.
///
/// Panics with a descriptive message naming the offending shader(s) if any watchdog fired.
/// Cleans up (frees) the watchdog buffer regardless of outcome.
///
/// # Shader flag bits
/// - Bit 0 (`0x1`): `lbvh_prepass.comp` — init pass watchdog
/// - Bit 1 (`0x2`): `lbvh_build.comp` — Karras hierarchy builder (`determine_range`/`find_split`)
/// - Bit 2 (`0x4`): `lbvh_build_bottomup.comp` — bottom-up aggregation or corrupt parent pointer
#[cfg(test)]
pub fn check_bvh_watchdog(allocator: vk_mem::AllocatorView) {
  LAST_BVH_WATCHDOG_STATE.with(|cell| {
    let mut guard = cell.lock();
    if let Some((buf, mut alloc, _addr)) = guard.take() {
      let info = allocator.get_allocation_info(&alloc);
      let flags = if !info.mapped_data.is_null() {
        unsafe { *(info.mapped_data as *const u32) }
      } else {
        0
      };
      unsafe { allocator.destroy_buffer(buf, &mut alloc) };
      ...
      if flags != 0 {
        ...
        panic!(
          "SHADER WATCHDOG TRIPPED — safety counter hit in: {}. \
           ...
           Check the BVH tree for cycles or degenerate Morton codes.",
          names.join(", ")
        );
      }
    }
...
pub struct PhysicsPipelines {
  ...
  /// External gravity emitters → BVH cluster force accumulation (Phase A)
  pub apply_emitters_to_particles: vk::Pipeline,
  /// Direct per-particle external gravity (BVH-free, for test-particle systems)
  pub apply_emitters_direct: vk::Pipeline,
  /// BVH cluster forces → per-particle AOSOA force slots (Phase B, leaf→root splat)
  pub accumulate_bvh_forces_to_particles: vk::Pipeline,
  // ── Narrow Phase ──────────────────────────────────────────────────────────
  #[cfg(any(test, feature = "collisions"))]
  ...
  pub fn new(
    ...
    let res: GpuResult<Self> = (|| {
      Ok(Self {
        ...
        apply_emitters_direct: mk_wg!("apply_emitters_direct.comp"),
        accumulate_bvh_forces_to_particles: mk_wg!("accumulate_bvh_forces_to_particles.comp"),
...
impl VulkanComputeKernels {
  ...
  pub fn cleanup(&mut self, device: &LogicalDevice, allocator: vk_mem::AllocatorView) {
    ...
    // to avoid the "allocations not freed before block destruction" assertion.
    #[cfg(test)]
    {
      LAST_BVH_WATCHDOG_STATE.with(|cell| {
        let mut guard = cell.lock();
...
impl VulkanComputeKernels {
  ...
  fn build_particles(
    ...
    scene0.query2::<crate::scene::TransformComponent, crate::scene::particles::ParticleSystemComponent, _>(
      |entity, _transform, sys| {
        ...
        let mut meta: alloc::vec::Vec<gpu::ParticleMetadata> = alloc::vec::Vec::new();

        for (i, p) in particles_guard.iter().enumerate().filter(|(_, p)| p.active != 0) {
          flat.push(alloc::vec![
          ...
          meta.push(gpu::ParticleMetadata {
            entity_id: entity,
        ...
        if !flat.is_empty() {
          systems.push((entity, dense_frame_idx, flat, meta, sys.disable_self_gravity));
        }
      },
    );

    for (entity, _frame_idx, flat_particles, metadata, disable_self_gravity) in systems {
      let actual_count = flat_particles.len() as u32;
      ...
      buffer.actual_count = actual_count;
      result.push((entity, buffer, metadata, disable_self_gravity));
    }
      ...
      .query2::<crate::scene::TransformComponent, crate::scene::ParticleEmitterCirclesComponent, _>(
        |_, _t, emitter| {
          for circle in &emitter.circles {
            ...
            // Approximate the candidate count from the continuous rate at 60 Hz.
            let approx_count = (circle.particles_per_second / 60.0).ceil() as u32;
  ...
  /// Dispatches `accumulate_bvh_forces_to_particles.comp` (Phase B).
  ///
  /// After `compute_self_gravity` and `apply_emitters_to_particles` have both
  /// written forces into BVH node slots, this shader traverses from each
  /// particle's leaf up to the root, summing all cluster forces and
  /// atomically accumulating the result into AOSOA force slots 7-9.
  ///
  /// Must be called after a pipeline barrier that follows Phase A.
  pub fn accumulate_bvh_forces_to_particles(
    &self,
    device: &LogicalDevice,
    cmd: &mut VulkanCommandBuffer,
    particles_addr: u64,
    ...
    if total_particles == 0 || bvh_addr == 0 {
      return;
    }
    let wg_size = self.effective_wg(128);
    let groups = (total_particles + wg_size - 1) / wg_size;

    let pc = crate::gpu::compute_push_constants::AccumBvhForcesPushConstants {
      particles: particles_addr,
      bvh: bvh_addr,
      total_particles,
      _pad: [0; 3],
    ...
    unsafe {
      device.cmd_bind_pipeline(
        cmd.cmd,
        vk::PipelineBindPoint::COMPUTE,
        self.pipelines.accumulate_bvh_forces_to_particles,
      );
      self.pipelines.assert_pc_size(
        self.pipelines.accumulate_bvh_forces_to_particles,
        bytes.len(),
      );
      device.cmd_push_constants(
  ...
  // ── apply_emitters_direct: BVH-free external gravity for test particles ─────
  fn apply_emitters_direct(
    ...
    cmd: &mut VulkanCommandBuffer,
    particles: &mut VulkanBuffer<f32>,
    emitters: &VulkanBuffer<crate::gpu::ForceEmitter>,
    frames: &VulkanBuffer<crate::physics::physics_scene::GpuReferenceFrame>,
    particle_frame_ids: &VulkanBuffer<u32>,
    num_emitters: u32,
  ) -> GpuResult<()> {
    if num_emitters == 0 || particles.actual_count == 0 {
      return Ok(());
    }
    let total_particles = particles.actual_count;
    let wg_size = self.effective_wg(128);
    let groups = (total_particles + wg_size - 1) / wg_size;

    let pc = crate::gpu::compute_push_constants::ApplyEmittersDirectPushConstants {
      particles: particles.address,
      emitters: emitters.address,
      frames: frames.address,
      particle_frame_ids: particle_frame_ids.address,
      num_emitters,
      total_particles,
      _pad: [0; 2],
    };
    ...
    unsafe {
      device.cmd_bind_pipeline(
        cmd.cmd,
        vk::PipelineBindPoint::COMPUTE,
        self.pipelines.apply_emitters_direct,
      );
      self.pipelines.assert_pc_size(self.pipelines.apply_emitters_direct, bytes.len());
      device.cmd_push_constants(
        cmd.cmd,
        self.pipelines.pipeline_layout,
        vk::ShaderStageFlags::COMPUTE,
  ...
  fn build_motion_bvh(
    ...
    let total_particles: u32 = if particles.actual_count > 0 {
      ...
      if !info.mapped_data.is_null() {
        ...
        let count_ptr = info.mapped_data as *const u32;
        let count = unsafe { *count_ptr.add(3) };
        count
      } else {
        // Fallback: derive from capacity (pre-mapped case, should not happen in practice)
        let sg = self.pipelines.subgroup_size;
        let stride = gpu::PARTICLE_FIELDS as u32 * sg;
        (particles.capacity() as u32 / stride) * sg
      }
    ...
    // completion (60 s timeout — hang = that shader is the culprit), then
    // allocates a fresh command buffer for the next dispatch.
    // Optionally pass a host-visible VulkanBuffer<u32> as second arg to dump
    // the first 8 u32s for data inspection.
    #[cfg(feature = "shader_debug_sync")]
    macro_rules! bvh_sync {
      ($label:expr) => {{
      ...
      ($label:expr, dump: $buf:expr, $n:expr) => {{
        bvh_sync!($label);
        // Dump first $n u32s from a host-visible buffer for data inspection
        let info = allocator.get_allocation_info(&$buf.allocation);
        if !info.mapped_data.is_null() {
          let ptr = info.mapped_data as *const u32;
          let cap = ($buf.capacity() as usize);
          let count = ($n as usize).min(cap);
          let slice = unsafe { core::slice::from_raw_parts(ptr, count) };
          aethervk_oshal_rlib::log!("[BVH-SYNC] {} data[0..{}]: {:?}", $label, count, slice);
        } else {
          aethervk_oshal_rlib::log!("[BVH-SYNC] {} buffer not host-visible, cannot dump", $label);
        }
      }};
    }
    ...
    // ── Allocate a small host-visible watchdog buffer for shader hang detection ───────────
    // In test builds the CPU reads this after GPU completion to detect which shader tripped
    // its safety counter. Bit 0 = lbvh_prepass, Bit 1 = lbvh_build, Bit 2 = lbvh_bottomup.
    #[cfg(test)]
    let watchdog_buf = {
      ...
      alloc_info.flags =
        vk_mem::AllocationCreateFlags::HOST_ACCESS_RANDOM | vk_mem::AllocationCreateFlags::MAPPED;
      let (buf, mut alloc) = unsafe { allocator.create_buffer(&buf_info, &alloc_info) }
        .map_err(|e| GpuError::BackendSpecific(alloc::format!("watchdog alloc failed: {:?}", e)))?;
      // Zero the buffer before GPU access
      let info = allocator.get_allocation_info(&alloc);
      if !info.mapped_data.is_null() {
        unsafe { *(info.mapped_data as *mut u32) = 0u32 };
      }
      let addr_info = vk::BufferDeviceAddressInfo::default().buffer(buf);
      let addr = unsafe { device.buffer_device_address.get_buffer_device_address(&addr_info) };
      (buf, alloc, addr)
    };
    #[cfg(test)]
    let watchdog_out_addr = watchdog_buf.2;
    #[cfg(not(test))]
    let watchdog_out_addr: u64 = 0;

    let pc = LbvhPushConstants {
      bvh: bvh_buffer.address,
      sorted_morton: sorted_morton.address, // Must be a valid BDA — passing 0 causes speculative page faults on Pascal GPUs.
    ...
    unsafe {
      ...
      // Barrier: prepass writes → lbvh_build reads
    ...
    // ── 9. Collapse binary BVH → wide multi-BVH ──────────────────────────────
    // The binary Karras tree (n-1 internal + n leaf nodes) is collapsed into a
    ...
    let multi_bvh_buffer = {
      ...
      match self.pipelines.subgroup_size {
        128 => alloc_multi_buf!(128),
        64 => alloc_multi_buf!(64),
        32 => alloc_multi_buf!(32),
        16 => alloc_multi_buf!(16),
        8 => alloc_multi_buf!(8),
        4 => alloc_multi_buf!(4),
        _ => alloc_multi_buf!(32),
      }
    }?;
    ...
    #[cfg(test)]
    {
      let (wd_buf, wd_alloc, wd_addr) = watchdog_buf;
      LAST_BVH_WATCHDOG_STATE.with(|cell| {
        let mut guard = cell.lock();
        if let Some((prev_buf, mut prev_alloc, _)) = guard.take() {
          unsafe { allocator.destroy_buffer(prev_buf, &mut prev_alloc) };
        }
        *guard = Some((wd_buf, wd_alloc, wd_addr));
      });
    }
  ...
  fn snapshot_dynamics(
  ...
  ) -> GpuResult<(VulkanBuffer<RigidBodyImex>, Option<VulkanBuffer<f32>>)> {
    ...
    let p_snap: Option<VulkanBuffer<f32>> = if let Some(particles) = particles {
      let snap = self.allocate_device_buffer::<f32>(
        device,
        allocator,
        particles.capacity(),
        vk::BufferUsageFlags::STORAGE_BUFFER
  ...
  fn restore_dynamics(
    ...
    _rollback: &mut utils::RollbackContext<'_>,
    cmd: &mut VulkanCommandBuffer,
    _rigid_bodies: &mut VulkanBuffer<RigidBodyImex>,
    _particles: Option<&mut VulkanBuffer<f32>>,
    _snapshot: &(VulkanBuffer<RigidBodyImex>, Option<VulkanBuffer<f32>>),
  ) -> GpuResult<()> {
    unsafe {
      // TODO switch to synchronization2
      let barrier = vk::MemoryBarrier::default()
  ...
  fn write_back_to_scene(
   ...
   // All device buffers are allocated with HOST_VISIBLE + HOST_COHERENT + MAPPED.
    ...
    // Process each particle system INDEPENDENTLY.
    // Each system has its own AoSoA buffer and its own BVH permutation stored under its entity.
    // We cannot concatenate multiple AoSoA buffers and unpack as one — AoSoA packing is
    // relative to each buffer's base, so concatenation produces garbage for all but the first.
    //
    ...
    scene.query2_mut::<crate::scene::TransformComponent, crate::scene::particles::ParticleSystemComponent, _>(|entity, _transform, sys| {
      let (gpu_buf, sys_meta) = match sys_map.get(&entity) {
        Some((buf, meta)) => (*buf, *meta),
        None => return, // no GPU buffer was built for this entity this frame
      };

      // Get this system's raw AoSoA data.
      let p_data: &[f32] = unsafe { gpu_buf.mapped_slice().unwrap_or(&[]) };

      // Unpack THIS SYSTEM's AoSoA buffer independently (sys_meta.len() particles, subgroup_size lanes).
      let unpacked = gpu::unpack_particles_aosoa(p_data, sg, gpu::PARTICLE_FIELDS, sys_meta.len());

      // Retrieve BVH permutation for this entity (maps GPU slot → original metadata index within THIS system).
      let permutation: alloc::vec::Vec<u32> = {
        ...
        if let Some((buf, _stamp)) = perm_map.remove(&entity) {
          ...
          // No BVH built for this entity this frame — identity permutation (system-local).
          (0..sys_meta.len() as u32).collect()
        }
      };
      ...
      for (gpu_slot, p_gpu) in unpacked.iter().enumerate() {
        // Map this GPU slot back to the original metadata index within this system.
        let orig_local_idx = permutation.get(gpu_slot).copied().unwrap_or(gpu_slot as u32) as usize;
        let meta = match sys_meta.get(orig_local_idx) {
          Some(m) => m,
          None => continue,
        };
        // entity_id check: only write back if this metadata belongs to this system.
        ...
        if (meta.original_index as usize) < sys_particles.len() {
          ...
          sys_sort_order.push(orig_local_idx as u32);
          alive_count += 1;
        }
      }
...
impl Kernels for Device {
  ...
  fn apply_emitters_direct(
    ...
    utils::VulkanTransaction::new(&*self.res, &self.device)
      ...
      .execute(|allocator, rollback| {
        self.kernels.apply_emitters_direct(
          &self.device,
          allocator,
          rollback,
          cmd,
    ...
    utils::VulkanTransaction::new(&*self.res, &self.device)
      ...
      .execute(|(), _rollback| {
        ...
        self.kernels.apply_emitters_to_particles(
          ...
          <Self::MotionBvh as crate::gpu::DeviceBvh>::address(bvh),
          num_emitters,
          total_particles,
        );
        Ok(())
