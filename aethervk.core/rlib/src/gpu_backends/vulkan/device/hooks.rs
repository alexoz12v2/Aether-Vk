...
pub static CMD_BUF_TO_DEVICE: Mutex<BTreeMap<vk::CommandBuffer, vk::Device>> =
...
impl DispatchableToDevice for vk::Queue {
  fn to_device(&self) -> vk::Device {
    *QUEUE_TO_DEVICE.lock().get(self).expect("vk::Queue not found in tracking map")
  }
}
...
pub static GET_DEVICE_QUEUE_PTRS: Mutex<BTreeMap<vk::Device, vk::PFN_vkGetDeviceQueue>> =
  Mutex::new(BTreeMap::new());
pub static GET_DEVICE_QUEUE2_PTRS: Mutex<BTreeMap<vk::Device, vk::PFN_vkGetDeviceQueue2>> =
  Mutex::new(BTreeMap::new());
...
#[allow(non_snake_case)]
pub unsafe extern "system" fn hooked_vkGetDeviceQueue2(
  device: vk::Device,
  p_queue_info: *const vk::DeviceQueueInfo2,
  p_queue: *mut vk::Queue,
) {
  assert_no_locks_held();
  let real_fn = *GET_DEVICE_QUEUE2_PTRS.lock().get(&device).expect("vkGetDeviceQueue2 missing");
  unsafe { real_fn(device, p_queue_info, p_queue) };
  QUEUE_TO_DEVICE.lock().insert(unsafe { *p_queue }, device);
}

pub static ALLOCATE_COMMAND_BUFFERS_PTRS: Mutex<
  BTreeMap<vk::Device, vk::PFN_vkAllocateCommandBuffers>,
> = Mutex::new(BTreeMap::new());
...
pub static FREE_COMMAND_BUFFERS_PTRS: Mutex<BTreeMap<vk::Device, vk::PFN_vkFreeCommandBuffers>> =
  Mutex::new(BTreeMap::new());
...
pub unsafe extern "system" fn hooked_vkFreeCommandBuffers(
  ...
  let buffers =
    unsafe { core::slice::from_raw_parts(p_command_buffers, command_buffer_count as usize) };
  let mut map = CMD_BUF_TO_DEVICE.lock();
  for &cb in buffers {
    map.remove(&cb);
  }
