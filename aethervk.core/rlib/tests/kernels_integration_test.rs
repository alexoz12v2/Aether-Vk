use aethervk_core_rlib::{
  ...
  scene::{Scene, TransformComponent, KinematicComponent, ReferenceFrameComponent},
...
fn setup_test_scene() -> Scene {
  ...
  let _ = scene.add_component(e1, TransformComponent {
    position: Vec3f32::from_array([0.0, 10.0, 0.0]),
    ..Default::default()
  });
  let _ = scene.add_component(e1, KinematicComponent {
    velocity: Vec3f32::from_array([0.0, -9.8, 0.0]),
    ..Default::default()
  });
  let _ = scene.add_component(e1, aethervk_core_rlib::scene::PhysicalMeshComponent {
  ...
  let _ = scene.add_component(e2, TransformComponent {
    position: Vec3f32::from_array([0.0, -10.0, 0.0]),
    ..Default::default()
  });
  let _ = scene.add_component(e2, KinematicComponent {
    velocity: Vec3f32::from_array([0.0, 9.8, 0.0]),
    ..Default::default()
  });
  let _ = scene.add_component(e2, aethervk_core_rlib::scene::PhysicalMeshComponent {
    asset_path: "".to_string(),
    mesh: comet.clone(),
    emissive_intensity: 0.0,
    emissive_color: [0.0; 3],
  ...
  let _ = scene.add_component(macro_frame, TransformComponent {
    ..Default::default()
  });
  let _ = scene.add_component(macro_frame, ReferenceFrameComponent {
...
fn test_imex_collision_with_lca() {
  ...
  scene.add_component(lca_entity, TransformComponent {
    position: Vec3f32::from_array([10.0, 0.0, 0.0]),
    ..Default::default()
  });
  scene.add_component(lca_entity, ReferenceFrameComponent {
