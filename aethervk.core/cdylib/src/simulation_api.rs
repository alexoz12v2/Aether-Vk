...
static BREADCRUMB_CALLBACK: core::sync::atomic::AtomicPtr<()> = core::sync::atomic::AtomicPtr::new(core::ptr::null_mut());
...
pub unsafe extern "C" fn avkSimulationContext_loadAlmanacFile(
  ...
  if let Ok(data) = oshal::os::fs::read(path_buf.as_ref()) {
    ...
    if let Ok(new_almanac) = logic.almanac_data.almanac.clone().load_from_bytes(bytes, path_str) {
...
pub unsafe extern "C" fn avkSimulationContext_getAlmanacLoadedFiles(
  ...
  let ctx = unsafe { &mut *ctx };
  let logic = ctx.logic_state.read();
  if !count.is_null() { unsafe { *count = logic.almanac_data.file_names.len() as u32; } }
  
  let mut ptrs: Vec<*mut c_char> = logic.almanac_data.file_names
    .iter()
...
pub unsafe extern "C" fn avkSimulationContext_getSimulationTimeUtc(
  ...
  if ctx.is_null() || buffer.is_null() || buffer_len == 0 { return false; }
  let ctx = unsafe { &mut *ctx };
  let logic = ctx.logic_state.read();
...
pub unsafe extern "C" fn avkSimulationContext_raycastNdc(
  ...
  ctx.scene.with_component(ctx.camera_entity, |c: &TransformComponent| {
    cam_pos = c.position;
    ...
    ) * <Mat4x4f32 as aethervk_oshal_rlib::math::matrix::Matrix4>::translation(c.position * -1.0);
  });

  ctx.scene.with_component(ctx.camera_entity, |cam: &CameraComponent| {
    let proj = cam.projection;
    let view_proj = proj * view;
    view_proj_inv = view_proj.inverse().unwrap_or(Mat4x4f32::identity());
  });

  let ndc_near = aethervk_oshal_rlib::math::vector::vec4::Vec4f32::from_components(ndc_x, ndc_y, 0.0, 1.0);
  let ndc_far  = aethervk_oshal_rlib::math::vector::vec4::Vec4f32::from_components(ndc_x, ndc_y, 1.0, 1.0);

  let mut world_near = view_proj_inv.mul_vector(ndc_near);
  let mut world_far  = view_proj_inv.mul_vector(ndc_far);

  if world_near.w() != 0.0 { world_near = world_near / world_near.w(); }
  if world_far.w()  != 0.0 { world_far  = world_far  / world_far.w(); }
  ...
  unsafe {
    avkSimulationContext_raycast(ctx, ro.x(), ro.y(), ro.z(), rd.x(), rd.y(), rd.z(), out_hit_entity, out_px, out_py, out_pz)
  }
}
...
pub unsafe extern "C" fn avkSimulationContext_raycast(
  ...
  ctx.scene.query2::<PhysicalMeshComponent, TransformComponent, _>(|entity, mesh, transform| {
    ...
    let local_ro = inv_model.mul_vector(aethervk_oshal_rlib::math::vector::vec4::Vec4f32::from_components(ro.x(), ro.y(), ro.z(), 1.0));
    let local_rd = inv_model.mul_vector(aethervk_oshal_rlib::math::vector::vec4::Vec4f32::from_components(rd.x(), rd.y(), rd.z(), 0.0));
    ...
    for i in (0..mesh.mesh.indices.len()).step_by(3) {
      if i + 2 >= mesh.mesh.indices.len() { break; }
      ...
      if t > 1e-5 && t < closest_t {
        ...
        let global_hit = model_matrix.mul_vector(aethervk_oshal_rlib::math::vector::vec4::Vec4f32::from_components(local_hit.x(), local_hit.y(), local_hit.z(), 1.0));
  ...
  if let Some(entity) = hit_entity {
    ...
    if !out_hit_entity.is_null() { unsafe { *out_hit_entity = external_id; } }
    if !out_px.is_null() { unsafe { *out_px = hit_point.x(); } }
    if !out_py.is_null() { unsafe { *out_py = hit_point.y(); } }
    if !out_pz.is_null() { unsafe { *out_pz = hit_point.z(); } }
    return true;
  }
...
pub unsafe extern "C" fn avkSimulationContext_setMarkers(
  ...
  entity: u64,
  count: u32,
  px: *const f32, py: *const f32, pz: *const f32,
  cr: *const f32, cg: *const f32, cb: *const f32,
  sizes: *const f32,
) {
  if ctx.is_null() || px.is_null() || py.is_null() || pz.is_null() || cr.is_null() || cg.is_null() || cb.is_null() || sizes.is_null() { return; }
  let ctx = unsafe { &mut *ctx };
  if let Some(entity_id) = ctx.get_entity(entity) {
    ...
    let _ = ctx.scene.add_component(entity_id, aethervk_core_rlib::scene::MarkersComponent { markers });
  }
}
...
pub unsafe extern "C" fn avkSimulationContext_getEpochLimits(
  ...
  let ctx = unsafe { &mut *ctx };
  let logic = ctx.logic_state.read();
  if !start_tai.is_null() { unsafe { *start_tai = logic.epoch_start.to_tai_seconds(); } }
  if !end_tai.is_null() { unsafe { *end_tai = logic.epoch_end.to_tai_seconds(); } }
  true
}
...
pub unsafe extern "C" fn avkSimulationContext_setTransformComponent(
  ...
  let ctx = unsafe { &mut *ctx };
  if let Some(entity_id) = ctx.get_entity(entity) {
    ctx.scene.with_component_mut(entity_id, |c: &mut TransformComponent| {
...
pub unsafe extern "C" fn avkSimulationContext_getTransformComponent(
  ...
  if let Some(entity_id) = ctx.get_entity(entity) {
    if let Some(transform) = ctx.scene.global_transform(entity_id) {
      if !pos_x.is_null() { unsafe { *pos_x = transform.position.x(); } }
      if !pos_y.is_null() { unsafe { *pos_y = transform.position.y(); } }
      if !pos_z.is_null() { unsafe { *pos_z = transform.position.z(); } }
      if !rot_w.is_null() { unsafe { *rot_w = transform.rotation.scalar_part(); } }
      let v = transform.rotation.vector_part();
      if !rot_x.is_null() { unsafe { *rot_x = v.x(); } }
      if !rot_y.is_null() { unsafe { *rot_y = v.y(); } }
      if !rot_z.is_null() { unsafe { *rot_z = v.z(); } }
      if !scale_x.is_null() { unsafe { *scale_x = transform.scale.x(); } }
      if !scale_y.is_null() { unsafe { *scale_y = transform.scale.y(); } }
      if !scale_z.is_null() { unsafe { *scale_z = transform.scale.z(); } }
      return true;
    }
  }
  false
...
pub unsafe extern "C" fn avkSimulationContext_setCameraComponent(
  ...
  let ctx = unsafe { &mut *ctx };
  if let Some(entity_id) = ctx.get_entity(entity) {
    ctx.scene.with_component_mut(entity_id, |c: &mut CameraComponent| {
      if is_orthographic {
...
pub unsafe extern "C" fn avkSimulationContext_getCameraComponent(
  ...
  if let Some(entity_id) = ctx.get_entity(entity) {
    ...
    ctx.scene.with_component(entity_id, |c: &CameraComponent| {
      ...
      slice[0] = c0.x(); slice[1] = c0.y(); slice[2] = c0.z(); slice[3] = c0.w();
      slice[4] = c1.x(); slice[5] = c1.y(); slice[6] = c1.z(); slice[7] = c1.w();
      slice[8] = c2.x(); slice[9] = c2.y(); slice[10] = c2.z(); slice[11] = c2.w();
      slice[12] = c3.x(); slice[13] = c3.y(); slice[14] = c3.z(); slice[15] = c3.w();
      success = true;
    });
    return success;
  }
...
pub unsafe extern "C" fn avkSimulationContext_processCommand(
  ...
  match command.cmd_type {
    ...
    3 => {
      ...
      ctx.scene.with_component_mut(ctx.cursor_entity, |c: &mut TransformComponent| {
        c.position = c.position + translation;
      });
      ctx.scene.with_component_mut(ctx.camera_entity, |c: &mut TransformComponent| {
        c.position = c.position + translation;
      });
    }
    4 => {
    ...
    5 => {
      ...
      // amount is not passed separately, let's just use the axis as delta vector directly
      // Wait, let's just pass delta_x, delta_y, delta_z
      ...
      // We don't move camera in MoveCursor
    }
    _ => {}
  }
}
