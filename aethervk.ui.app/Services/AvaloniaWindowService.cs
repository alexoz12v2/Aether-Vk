...
namespace AetherVk.Services
{
    public class AvaloniaWindowService : IWindowService
    {
        ...
        {
            var mainWindow = GetMainWindow();
            if (mainWindow == null) return Task.CompletedTask;

            var isLightTheme = Application.Current?.ActualThemeVariant == Avalonia.Styling.ThemeVariant.Light;
            var vm = mainWindow.DataContext as MainWindowViewModel;
            var model = vm != null ? System.Linq.Enumerable.FirstOrDefault(vm.ImportedModels, m => m.Id.ToString() == meshId) : null;
        ...
        public async Task ShowSpawnMeshDialogAsync(string modelId, string modelName)
        {
            var mainWindow = GetMainWindow();
            if (mainWindow == null) return;
            ...
            {
                await _runtimeService.SpawnModelInstanceAsync(
                    1,
                    ulong.Parse(modelId),
                    vm.EntityName,
                    vm.PosX,
